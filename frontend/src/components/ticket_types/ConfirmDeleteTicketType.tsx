...
import {deleteTicketType} from "@/api_service/ticketType.ts";
import {Box, Button, HStack, Text, VStack} from "@chakra-ui/react";
import {FiAlertCircle, FiAlertTriangle} from "react-icons/fi";

export default function ConfirmDeleteTicketType({ id, onClose }: { id: number; onClose?: () => void }) {
    const queryClient = useQueryClient();

    const mutation = useMutation({
        mutationFn: () => deleteTicketType(id),
        onSuccess: async () => {
    ...
    return (
        <VStack gap={6} align="stretch">
            ...
            <HStack gap={3} justify="flex-end">
                <Button variant="ghost" colorPalette="gray" disabled={mutation.isPending} onClick={onClose}>
                    Cancel
                </Button>
                <Button colorPalette="red" onClick={() => mutation.mutate()} disabled={mutation.isPending}>
